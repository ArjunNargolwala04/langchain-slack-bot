...
# in-memory set to deduplicate retried events
...
async def slack_events(request: Request) -> JSONResponse:
    ...
    # Use thread_ts if in thread, o.w. use message ts to start new thread
    ...
    # Acknowledge immediately, then process in bg
...
async def _process_message(
    ...
    try:
        ...
        # Convert mkdown to Slack format
        answer = _format_for_slack(answer)

        # Post final answer, splitting if it exceeding limit
...
def _run_agent_with_progress(
    ...
    for event in agent.stream(
        ...
        # After tools node runs, update Slack
        if "tools" in event:
        ...
        # Verify node overrides with confidence-scored answer
        if "verify" in event:
